...
const InventoryManager: React.FC<InventoryManagerProps> = ({ inventory, onAdd, onUpdate, onDelete }) => {
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-2">Track parts, stock levels, and pricing.</p>
        </div>
        <button 
      ...
      {showForm && (
        <div className="bg-white p-8 rounded-lg shadow-md border-t-4 border-brand-red animate-in fade-in slide-in-from-top-4">
          <h3 className="text-xl font-bold text-brand-blue mb-6">{editingItem ? 'Edit Item' : 'Add New Inventory Item'}</h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Part Name</label>
              <input 
                type="text"
                required
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Part Number (SKU)</label>
              <input 
                type="text"
                required
                className="w-full p-3 border border-gray-200 rounded text-sm focus:border-brand-red outline-none"
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Min Stock Alert</label>
              <input 
                type="number"
                required
                min="0"
            ...
            <div className="md:col-span-2 lg:col-span-3 flex justify-end gap-4 mt-4">
              <button type="button" onClick={resetForm} className="px-6 py-3 rounded text-gray-600 font-bold text-xs uppercase hover:bg-gray-100">Cancel</button>
              <button type="submit" className="bg-brand-blue text-white px-8 py-3 rounded font-bold text-xs uppercase hover:bg-blue-900 shadow-md">Save Item</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-sm border-l-4 border-blue-500 flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-bold text-brand-blue">{inventory.length}</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border-l-4 border-green-500 flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-bold text-brand-blue">${inventory.reduce((acc, item) => acc + (item.price * item.quantity), 0).toLocaleString()}</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border-l-4 border-red-500 flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-bold text-brand-red">{inventory.filter(i => i.quantity <= i.minStockLevel).length}</p>
          </div>
          ...
          <table className="w-full text-sm text-left text-gray-600">
            ...
            <tbody>
              {filteredInventory.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-50 transition-colors group">
                  <td className="px-6 py-4">
                    {item.quantity <= 0 ? (
                      <span className="bg-red-100 text-red-700 text-[10px] font-bold px-2 py-1 rounded-full">Out of Stock</span>
                    ) : item.quantity <= item.minStockLevel ? (
                      <span className="bg-yellow-100 text-yellow-700 text-[10px] font-bold px-2 py-1 rounded-full">Low Stock</span>
                    ) : (
                      <span className="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-1 rounded-full">In Stock</span>
                    )}
                  </td>
                  <td className="px-6 py-4 font-bold text-brand-blue">{item.name}</td>
                  ...
                  <td className="px-6 py-4 flex justify-end gap-2">
                    <button onClick={() => handleEdit(item)} className="p-2 text-gray-400 hover:text-brand-blue hover:bg-blue-50 rounded-full transition-colors">
                      <Edit size={16} />
                    </button>
                    <button onClick={() => onDelete(item.id)} className="p-2 text-gray-400 hover:text-brand-red hover:bg-red-50 rounded-full transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </td>
                </tr>
              ...
              {filteredInventory.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-gray-400 italic">No inventory items found</td>
                </tr>
              )}
            </tbody>
          </table>
